...
class BayesianInference(object):
    ...
    @classmethod
    def ep(cls, n, comparisons, prior, max_iter=MAX_ITERATIONS, state=None):
        return _ep(n, comparisons, prior, cls.match_moments, max_iter, state)


class Logit(BayesianInference):
    ...
    def match_moments(cls, mean_cav, cov_cav):
        ...
        for i, x in enumerate(lambdas):
            arr1[i], arr2[i], arr3[i] = Probit.match_moments(x * mean_cav,
                                                             x*x * cov_cav)
...
class Probit(BayesianInference):
...
def _logphi(z):
    ...
    if z * z < 0.0492:
        ...
        coef = -z / SQRT2PI
        val = reduce(lambda acc, c: coef * (c + acc), CS, 0)
        res = -2 * val - log(2)
        dres = exp(-(z * z) / 2 - res) / SQRT2PI
    elif z < -11.3137:
        # Second case: z very small.
        num = reduce(lambda acc, r: -z * acc / SQRT2 + r, RS,
                     0.5641895835477550741)
        den = reduce(lambda acc, q: -z * acc / SQRT2 + q, QS, 1.0)
...
def _ep(n, comparisons, prior, match_moments, max_iter=MAX_ITERATIONS, state=None):
