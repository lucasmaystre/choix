...
def _mm(num_items, data, initial_params, alpha, max_iter, tol, mm_fun):
    ...
    for _ in range(max_iter):
        nums, denoms = mm_fun(num_items, data, params)
        params = (nums + alpha) / (denoms + alpha)
        params = (num_items / params.sum()) * params
        if converged(params):
            return params
    raise RuntimeError("Did not converge after {} iterations".format(max_iter))


def _mm_pairwise(num_items, data, params):
    """Inner loop of MM algorithm for pairwise data."""
    wins = np.zeros(num_items, dtype=float)
    denoms = np.zeros(num_items, dtype=float)
...
def mm_pairwise(num_items, data, initial_params=None, alpha=0.0,
    ...
    """
    return _mm(num_items, data, initial_params, alpha, max_iter, tol,
            _mm_pairwise)
