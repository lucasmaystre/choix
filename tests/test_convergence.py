...
def test_nod_no_update():
    ...
    assert not test(np.exp([-1.0, +1.0]))
    # Average norm of difference = 0.5.
    assert test(np.exp([-0.5, +0.5]), update=False)
    # Average norm of difference = 0.8.
    assert not test(np.exp([-0.2, +0.2]))
    # Average norm of difference = 0.1.
    assert test(np.exp([-0.1, +0.1]))
...
def test_nod_normalization():
    ...
    for i in (1, 2, 5, 10):
        ...
        assert not test(np.exp([-1.7, 1.7] * i))
        assert not test(np.exp([-1.0, 1.0] * i))
        assert test(np.exp([-0.8, 0.8] * i))
